...
// -----------------------------------------------------------------------------
#ifndef __GP_RQ_KERNEL_H_
#define __GP_RQ_KERNEL_H_
...
class GpRqKernel {
 ...
 public:
  GpRqKernel(const Eigen::MatrixXd &xdata, const Eigen::VectorXd &ydata,
             const Eigen::VectorXd &amp, const Eigen::MatrixXd &ls,
             const Eigen::VectorXd &kappa, const Eigen::VectorXd &sigma,
             const double jitter);
...
// Rq kernel GP class-constructor
//
// Inputs:
//     xdata : (n,d) array
...
//
GpRqKernel::GpRqKernel(const Eigen::MatrixXd &xdata,
                       const Eigen::VectorXd &ydata, const Eigen::VectorXd &amp,
                       const Eigen::MatrixXd &ls, const Eigen::VectorXd &kappa,
  ...
  alphas.reserve(s);
  for (int i = 0; i < s; i++) {
    lxxs.push_back(chol_(
        covRq_(x, amp_sqs(i), ls_sqs.row(i), kappas(i), sigma_sqs(i), jitter)));
...
//         n points in d dimensions
//
const Eigen::MatrixXd GpRqKernel::view_x() { return x; }
...
//         n scalar responses
//
const Eigen::VectorXd GpRqKernel::view_y() { return y; }
...
GpRqKernel::view_parameters() {
  return std::make_tuple(amp_sqs, ls_sqs, kappas, sigma_sqs);
}
...
//
std::tuple<Eigen::MatrixXd, std::vector<Eigen::MatrixXd>> GpRqKernel::posterior(
    Eigen::Ref<const Eigen::MatrixXd> xstar) {
  int m = xstar.rows();
  ...
  for (int i = 0; i < s; i++) {
    kxxstar = crossCovRq_(x, xstar, amp_sqs(i), ls_sqs.row(i), kappas(i));
    means.col(i) = kxxstar.transpose() * alphas[i];
    covmats.push_back(conditionalCov(covRq_(xstar, amp_sqs(i), ls_sqs.row(i),
                                            kappas(i), sigma_sqs(i), jitter),
                                     lxxs[i], kxxstar));
  }
...
GpRqKernel::posterior_chol(Eigen::Ref<const Eigen::MatrixXd> xstar) {
  int m = xstar.rows();
  Eigen::MatrixXd means(n, s);
  std::vector<Eigen::MatrixXd> covmats;
  covmats.reserve(s);
  ...
  for (int i = 0; i < s; i++) {
    ...
    means.col(i) = kxxstar.transpose() * alphas[i];
    covmats.push_back(
        chol_(conditionalCov(covRq_(xstar, amp_sqs(i), ls_sqs.row(i), kappas(i),
                                    sigma_sqs(i), jitter),
                             lxxs[i], kxxstar))
            .matrixL());
  }
...
//         hyperparameter samples.
//
std::tuple<Eigen::MatrixXd, Eigen::MatrixXd> GpRqKernel::marginals(
    Eigen::Ref<const Eigen::MatrixXd> xstar) {
  int m = xstar.rows();
...
//
std::tuple<double, Eigen::VectorXd> GpRqKernel::E_jac(
  ...
  for (int i = 0; i < s; i++) {
    kxstarx = crossCovRq1d_(xstar_j, x, amp_sqs(i), ls_sqs.row(i), kappas(i));
    expectation += kxstarx.dot(alphas[i]);
    for (int idx = 0; idx < d; idx++) {
      // dE/dx_idx = dK/dx * alpha
      expectation_jac(idx) -=
          crossCovGradRq_(xstar_j, x, idx, amp_sqs(i), ls_sqs.row(i), kappas(i))
              .dot(alphas[i]);
    }
  }
...
Eigen::VectorXd GpRqKernel::px(Eigen::Ref<const Eigen::MatrixXd> xstar) {
  ...
  for (int i = 0; i < s; i++) {
    px.noalias() -= conditionalVar(
        ...
        crossCovRq_(x, xstar, amp_sqs(i), ls_sqs.row(i), kappas(i)), jitter);
  }
  return px / s;
}
...
//     marginal variance
//
std::tuple<Eigen::VectorXd, Eigen::VectorXd> GpRqKernel::px_jac(
    Eigen::Ref<const Eigen::RowVectorXd> xstar) {
  ...
  for (int i = 0; i < s; i++) {
    kxstarx = crossCovRq1d_(xstar, x, amp_sqs(i), ls_sqs.row(i), kappas(i));
    px.noalias() -=
        conditionalVar1d(amp_sqs(i) + sigma_sqs(i), lxxs[i], kxstarx, jitter);
    for (int idx = 0; idx < d; idx++) {
      px_jac(idx) +=
          kxstarx * lxxs[i].solve(-crossCovGradRq_(xstar, x, idx, amp_sqs(i),
                                                   ls_sqs.row(i), kappas(i))
                                       .transpose());
    }
  }
...
//     this function measures the expected reduction over the incumbent minimum.
//
std::tuple<double, Eigen::VectorXd> GpRqKernel::ei_jac(
    Eigen::Ref<const Eigen::RowVectorXd> xstar) {
  double ei = 0;
  double mean;
  double mean_grad;
  ...
  for (int i = 0; i < s; i++) {
    ...
    for (int idx = 0; idx < d; idx++) {
      dkxstarx_dxstar =
          crossCovGradRq_(xstar, x, idx, amp_sqs(i), ls_sqs.row(i), kappas(i));
      sd_grad =
...
//
Eigen::VectorXd GpRqKernel::ucb(Eigen::Ref<const Eigen::MatrixXd> xstar,
                                double beta) {
  int m = xstar.rows();
...
//     speaking this function measures a (negative) lower confidence bound.
//
std::tuple<double, Eigen::VectorXd> GpRqKernel::ucb_jac(
    Eigen::Ref<const Eigen::RowVectorXd> xstar, double beta) {
  double ucb = 0;
  double sd;
  double sr_beta = sqrt(beta);
...
double GpRqKernel::entropy(Eigen::Ref<const Eigen::MatrixXd> xstar) {
  ...
  for (int i = 0; i < s; i++) {
    ...
    h += tmp + (covRq_(xstar, amp_sqs(i), ls_sqs.row(i), kappas(i),
                       sigma_sqs(i), jitter)
                    .array() -
