    ...
    for(int i = 0; i < x.rows(); i++){
        for(int j = i; j < x.rows(); j++){
            if (i!=j){
                ...
                sr_tau = sqrt(tau);
                c(j, i) = amp_sq * (1.0 + sr_3 * sr_tau) * exp(-sr_3 * sr_tau);
            }
            if (i==j){
    ...
    Eigen::Ref<const Eigen::VectorXd> ls_sq)
{
    ...
    tmp = tau.array().sqrt() * sqrt(3.0);
    return amp_sq * (1.0 + tmp.array()) * (-tmp).array().exp();
}

Eigen::RowVectorXd cov_1d_m32(
    ...
    Eigen::Ref<const Eigen::VectorXd> ls_sq)
{
    ...
    for(int i = 0; i < x.rows(); i++){
        ...
        sr_tau = sqrt(tau);
        c(i) = amp_sq * (1 + sr_3 * sr_tau) * exp(-sr_3 * sr_tau);
    }
    return c;
}

Eigen::RowVectorXd cross_cov_grad_m32(
    Eigen::Ref<const Eigen::RowVectorXd> xstar1d,
    Eigen::Ref<const Eigen::MatrixXd> x,
    int d,
    double amp_sq,
        ...
        delta(jitter)
{
    for (int k=0; k<amp_sq.size(); k++){
        ...
        lxxs.push_back(L_cov_m32(x, amp_sq(k), ls_sq.row(k), sigma_sq(k)));
...
const Eigen::MatrixXd GpM32Kernel::view_x()
{
    return x;
}

// provide a view of y data
const Eigen::VectorXd GpM32Kernel::view_y()
{
    return y;
}

// provide a view of parameters
std::tuple<const Eigen::VectorXd, const Eigen::MatrixXd, const Eigen::VectorXd> GpM32Kernel::view_parameters()
{
...
// Posterior
std::tuple<Eigen::MatrixXd, std::vector<Eigen::MatrixXd>> GpM32Kernel::posterior(Eigen::Ref<const Eigen::MatrixXd> xstar)
{
    ...
    for (int k=0; k<amp_sq.size(); k++){
        kxxstar = cross_cov_m32(x, xstar, amp_sq(k), ls_sq.row(k));
        ...
        */
        v.push_back(
             (
                 cov_m32(xstar, amp_sq(k), ls_sq.row(k), sigma_sq(k)).array() -
...
// Posterior (Cholesky)
std::tuple<Eigen::MatrixXd, std::vector<Eigen::MatrixXd>> GpM32Kernel::posterior_chol(Eigen::Ref<const Eigen::MatrixXd> xstar)
{
    Eigen::MatrixXd m(xstar.rows(), amp_sq.size());
    std::vector<Eigen::MatrixXd> v;
    v.reserve(amp_sq.size());
...
// Posterior marginals
std::tuple<Eigen::MatrixXd, Eigen::MatrixXd> GpM32Kernel::marginals(Eigen::Ref<const Eigen::MatrixXd> xstar)
{
    Eigen::MatrixXd m(xstar.rows(), amp_sq.size());
...
std::tuple<double, Eigen::VectorXd> GpM32Kernel::E_jac(Eigen::Ref<const Eigen::RowVectorXd> xstar)
{
    ...
    for (int k=0; k<amp_sq.size(); k++){
        kxstarx = cov_1d_m32(xstar, x, amp_sq(k), ls_sq.row(k));
        m += kxstarx.dot(axxs[k]);
        for (int d=0; d<xstar.size(); d++){
            dkxstarx_dxstar = cross_cov_grad_m32(xstar, x, d, amp_sq(k), ls_sq.row(k));
...
std::tuple<Eigen::VectorXd, Eigen::VectorXd> GpM32Kernel::px_jac(Eigen::Ref<const Eigen::RowVectorXd> xstar)
{
    Eigen::RowVectorXd kxstarx(x.rows());
    Eigen::RowVectorXd dkxstarx_dxstar(x.rows());
...
std::tuple<double, Eigen::VectorXd> GpM32Kernel::ei_jac(Eigen::Ref<const Eigen::RowVectorXd> xstar)
{
    double ei = 0;
    double mean;
    double mean_grad;
...
Eigen::VectorXd GpM32Kernel::ucb(Eigen::Ref<const Eigen::MatrixXd> xstar, double beta)
{
    Eigen::MatrixXd kxxstar(x.rows(), xstar.rows());
    Eigen::VectorXd mean(xstar.rows());
    Eigen::VectorXd sd(xstar.rows());
...
std::tuple<double, Eigen::VectorXd> GpM32Kernel::ucb_jac(Eigen::Ref<const Eigen::RowVectorXd> xstar, double beta)
{
    double ucb = 0;
    double mean;
    double mean_grad;
...
double GpM32Kernel::entropy(Eigen::Ref<const Eigen::MatrixXd> xstar)
{
    ...
    for (int k=0; k<amp_sq.size(); k++){
        ...
        */
        h += tmp + ( cov_m32(xstar, amp_sq(k), ls_sq.row(k), sigma_sq(k)).array() -
