...
// Lower cholesky factor of Matern32 kernel sample covariance matrix
//
// Inputs:
//     x : (n, d) array
...
// Matern32 kernel 1d cross covariance calculation
//
// Inputs:
//     just_xstar : (d) array
...
//         dimensionwise (anisotropic) squared lengthscales
//     kxstarx : (n) array
//         Output of cov_1d_eq(just_xstar, x, amp_sq, ls_sq)
//
// Outputs:
//     dkxstarx_dxstar : (n) array
//         partial derivatives of k(just_xstar, x | amp_sq, ls_sq)
    ...
    Eigen::Ref<const Eigen::VectorXd> ls_sq)
{
    ...
    for(int i = 0; i < x.rows(); i++){
        ...
        dc(i) = -((xstar1d(d) - x(i,d)) / ls_sq(d)) *
            (amp_sq * -3.0 * exp(-(sqrt(tau) * sr_3)));
    }
    return dc;
}

// Class for a Gaussian Process with a Matern32 covariance kernel
//
// Private Attributes:
//     x : (n, d) array
//         n points in d dimensions
...
// Matern32 kernel GP class-constructor
//
// Inputs:
//     xdata : (n,d) array
//         n points in d dimensions
        ...
        delta(jitter)
{
    for (int k=0; k<amp_sq.size(); k++){
        ...
        lxxs.push_back(
             L_cov_m32(x, amp_sq(k), ls_sq.row(k), sigma_sq(k) + delta)
             );
    ...
    )
{
    ...
    for (int k=0; k<amp_sq.size(); k++){
         ...
         cov_mats.push_back(
              conditionalCov(
                   cov_m32(xstar, amp_sq(k), ls_sq.row(k), sigma_sq(k)),
                   lxxs[k],
                   kxxstar)
              );
    }
    ...
    )
{
    ...
    for (int k=0; k<amp_sq.size(); k++){
        ...
        for (int d=0; d<xstar.size(); d++){
            dkxstarx_dxstar = cross_cov_grad_m32(
                 xstar,
                 x,
                 d,
                 amp_sq(k),
    ...
    )
{
    ...
    for (int k=0; k < amp_sq.size(); k++){
        px.noalias() -= conditionalVar(
             amp_sq(k) + sigma_sq(k),
             lxxs[k],
             cross_cov_m32(x, xstar, amp_sq(k), ls_sq.row(k)),
             delta
             );
    }
