...
// Lower cholesky factor of Matern52 kernel sample covariance matrix
//
// Inputs:
//     x : (n, d) array
    ...
    Eigen::Ref<const Eigen::VectorXd> ls_sq)
{
    ...
    return amp_sq*(1.0 + sr_5*sr_tau.array() + rat*tau.array()) *
        (-sr_5*sr_tau).array().exp();
}

// Matern52 kernel 1d cross covariance calculation
//
// Inputs:
//     just_xstar : (d) array
...
// Matern52 kernel GP class-constructor
//
// Inputs:
//     xdata : (n,d) array
//         n points in d dimensions
        ...
        delta(jitter)
{
    for (int k=0; k<amp_sq.size(); k++){
        ...
        lxxs.push_back(
             L_cov_m52(x, amp_sq(k), ls_sq.row(k), sigma_sq(k) + delta)
             );
    ...
    )
{
    ...
    for (int k=0; k<amp_sq.size(); k++){
         ...
         cov_mats.push_back(
              conditionalCov(
                   cov_m52(xstar, amp_sq(k), ls_sq.row(k), sigma_sq(k)),
                   lxxs[k],
                   kxxstar)
              );
    }
    ...
    )
{
    ...
    for (int k=0; k<amp_sq.size(); k++){
        ...
        for (int d=0; d<xstar.size(); d++){
            dkxstarx_dxstar = cross_cov_grad_m52(
                 xstar,
                 x,
                 d,
                 amp_sq(k),
    ...
    )
{
    ...
    for (int k=0; k < amp_sq.size(); k++){
        px.noalias() -= conditionalVar(
             amp_sq(k) + sigma_sq(k),
             lxxs[k],
             cross_cov_m52(x, xstar, amp_sq(k), ls_sq.row(k)),
             delta
             );
    }
