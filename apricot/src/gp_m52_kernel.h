...
// -----------------------------------------------------------------------------
#ifndef __GP_M52_KERNEL_H_
#define __GP_M52_KERNEL_H_
...
class GpM52Kernel {
 ...
 public:
  GpM52Kernel(const Eigen::MatrixXd &xdata, const Eigen::VectorXd &ydata,
...
// M52 kernel GP class-constructor
//
// Inputs:
//     xdata : (n,d) array
...
//
GpM52Kernel::GpM52Kernel(const Eigen::MatrixXd &xdata,
  ...
  for (int i = 0; i < s; i++) {
    lxxs.push_back(
        chol_(covM52_(x, amp_sqs(i), ls_sqs.row(i), sigma_sqs(i), jitter)));
...
//         n points in d dimensions
//
const Eigen::MatrixXd GpM52Kernel::view_x() { return x; }
...
//         n scalar responses
//
const Eigen::VectorXd GpM52Kernel::view_y() { return y; }
...
GpM52Kernel::posterior(Eigen::Ref<const Eigen::MatrixXd> xstar) {
  int m = xstar.rows();
  ...
  for (int i = 0; i < s; i++) {
    kxxstar = crossCovM52_(x, xstar, amp_sqs(i), ls_sqs.row(i));
    means.col(i) = kxxstar.transpose() * alphas[i];
    covmats.push_back(conditionalCov(
        covM52_(xstar, amp_sqs(i), ls_sqs.row(i), sigma_sqs(i), jitter),
        lxxs[i], kxxstar));
  }
...
//
std::tuple<Eigen::MatrixXd, std::vector<Eigen::MatrixXd>>
GpM52Kernel::posterior_chol(Eigen::Ref<const Eigen::MatrixXd> xstar) {
  int m = xstar.rows();
  Eigen::MatrixXd means(n, s);
  std::vector<Eigen::MatrixXd> covmats;
  covmats.reserve(s);
  ...
  for (int i = 0; i < s; i++) {
    ...
    means.col(i) = kxxstar.transpose() * alphas[i];
    covmats.push_back(
        chol_(conditionalCov(covM52_(xstar, amp_sqs(i), ls_sqs.row(i),
                                     sigma_sqs(i), jitter),
                             lxxs[i], kxxstar))
            .matrixL());
  }
...
//         hyperparameter samples.
//
std::tuple<Eigen::MatrixXd, Eigen::MatrixXd> GpM52Kernel::marginals(
    Eigen::Ref<const Eigen::MatrixXd> xstar) {
  int m = xstar.rows();
...
//
std::tuple<double, Eigen::VectorXd> GpM52Kernel::E_jac(
  ...
  for (int i = 0; i < s; i++) {
    kxstarx = crossCovM521d_(xstar_j, x, amp_sqs(i), ls_sqs.row(i));
    expectation += kxstarx.dot(alphas[i]);
    for (int idx = 0; idx < d; idx++) {
      // dE/dx_idx = dK/dx * alpha
      expectation_jac(idx) -=
          crossCovGradM52_(xstar_j, x, idx, amp_sqs(i), ls_sqs.row(i))
              .dot(alphas[i]);
    }
  }
...
Eigen::VectorXd GpM52Kernel::px(Eigen::Ref<const Eigen::MatrixXd> xstar) {
  ...
  for (int i = 0; i < s; i++) {
    px.noalias() -= conditionalVar(
        ...
        crossCovM52_(x, xstar, amp_sqs(i), ls_sqs.row(i)), jitter);
  }
  return px / s;
}
...
//     marginal variance
//
std::tuple<Eigen::VectorXd, Eigen::VectorXd> GpM52Kernel::px_jac(
    Eigen::Ref<const Eigen::RowVectorXd> xstar) {
  ...
  for (int i = 0; i < s; i++) {
    kxstarx = crossCovM521d_(xstar, x, amp_sqs(i), ls_sqs.row(i));
    px.noalias() -=
        conditionalVar1d(amp_sqs(i) + sigma_sqs(i), lxxs[i], kxstarx, jitter);
    for (int idx = 0; idx < d; idx++) {
      px_jac(idx) +=
          kxstarx * lxxs[i].solve(-crossCovGradM52_(xstar, x, idx, amp_sqs(i),
                                                    ls_sqs.row(i))
                                       .transpose());
    }
  }
...
//     this function measures the expected reduction over the incumbent minimum.
//
std::tuple<double, Eigen::VectorXd> GpM52Kernel::ei_jac(
    Eigen::Ref<const Eigen::RowVectorXd> xstar) {
  double ei = 0;
  double mean;
  double mean_grad;
  ...
  for (int i = 0; i < s; i++) {
    ...
    for (int idx = 0; idx < d; idx++) {
      dkxstarx_dxstar =
          crossCovGradM52_(xstar, x, idx, amp_sqs(i), ls_sqs.row(i));
      sd_grad =
...
//
Eigen::VectorXd GpM52Kernel::ucb(Eigen::Ref<const Eigen::MatrixXd> xstar,
                                 double beta) {
  int m = xstar.rows();
...
//     speaking this function measures a (negative) lower confidence bound.
//
std::tuple<double, Eigen::VectorXd> GpM52Kernel::ucb_jac(
    Eigen::Ref<const Eigen::RowVectorXd> xstar, double beta) {
  double ucb = 0;
  double sd;
  double sr_beta = sqrt(beta);
...
double GpM52Kernel::entropy(Eigen::Ref<const Eigen::MatrixXd> xstar) {
  ...
  for (int i = 0; i < s; i++) {
    ...
    h += tmp + (covM52_(xstar, amp_sqs(i), ls_sqs.row(i), sigma_sqs(i), jitter)
                    .array() -
